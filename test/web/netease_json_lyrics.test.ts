...
const expected = "[00:00.342]文本作曲：某人";
if (parseNetEaseLyrics(payload) !== expected) throw new Error("parser did not create the expected timeline");
const credits = [
  JSON.stringify({ t: 0, c: [{ tx: "作词: " }, { tx: "某人" }] }),
  JSON.stringify({ t: 1000, c: [{ tx: "真正的歌词" }] }),
].join("\n");
if (parseNetEaseLyrics(credits) !== "[00:01.000]真正的歌词") throw new Error("credits must not become lyric lines");
