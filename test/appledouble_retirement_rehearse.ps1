...
function Scalar([string] $db, [string] $sql) {
  $value = & $sqlite $db $sql
  if ($LASTEXITCODE -ne 0) { throw "assertion failed: $sql" }
  return "$value".Trim()
}

$success = New-Fixture 'success'
$out = Invoke-Candidate $success $false
