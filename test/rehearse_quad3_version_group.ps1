...
$successRows = Invoke-D1Query $success "SELECT (SELECT COUNT(*) FROM album_display_group_members WHERE group_id='ag-q3-flac-wav-20260926') AS members, (SELECT COUNT(*) FROM song_masters WHERE album_id='al-q3-wav-20260926') AS wav_tracks, (SELECT COUNT(*) FROM song_masters WHERE album_id='al-1a27548730') AS flac_tracks, (SELECT duration FROM song_masters WHERE id='sm-upload-b8fabd82-f0b') AS retained_wav_duration, (SELECT song_count FROM albums WHERE id='al-q3-wav-20260926') AS wav_count, (SELECT duration FROM albums WHERE id='al-q3-wav-20260926') AS wav_duration, (SELECT size FROM albums WHERE id='al-q3-wav-20260926') AS wav_size, (SELECT song_count FROM albums WHERE id='pending-uploads') AS pending_count, (SELECT duration FROM albums WHERE id='pending-uploads') AS pending_duration, (SELECT size FROM albums WHERE id='pending-uploads') AS pending_size;"
$successText = $successRows | Out-String
if ($successText -notmatch '"members"\s*:\s*2' -or $successText -notmatch '"wav_tracks"\s*:\s*9' -or $successText -notmatch '"flac_tracks"\s*:\s*9' -or $successText -notmatch '"retained_wav_duration"\s*:\s*264' -or $successText -notmatch '"wav_count"\s*:\s*9' -or $successText -notmatch '"wav_duration"\s*:\s*2335' -or $successText -notmatch '"wav_size"\s*:\s*45' -or $successText -notmatch '"pending_count"\s*:\s*0' -or $successText -notmatch '"pending_duration"\s*:\s*0' -or $successText -notmatch '"pending_size"\s*:\s*0') { throw 'success verification failed' }
