...
$apply = Join-Path $repo 'agents\524_[Audit]_mixed_pending_album_repair\apply_qichengzhuanhe.sql'
$rollback = Join-Path $repo 'agents\524_[Audit]_mixed_pending_album_repair\rollback_qichengzhuanhe.sql'
...
$after = Query-D1 $success "SELECT (SELECT count(*) FROM song_masters WHERE album_id='al-524-qichengzhuanhe' AND track BETWEEN 1 AND 7) AS tracks,(SELECT cover_r2_key FROM albums WHERE id='al-524-qichengzhuanhe') AS cover,(SELECT count(*) FROM storage_objects) AS objects"
...
$rolled = Query-D1 $success "SELECT (SELECT count(*) FROM albums WHERE id='al-524-qichengzhuanhe') AS album,(SELECT count(*) FROM song_masters WHERE track IS NOT NULL) AS tracks,(SELECT count(*) FROM storage_objects) AS objects"
...
$blocked = Query-D1 $stale "SELECT (SELECT count(*) FROM albums WHERE id='al-524-qichengzhuanhe') AS album,(SELECT count(*) FROM song_masters WHERE id='sm-upload-05d63157-24d' AND album_id='pending-uploads' AND track IS NULL) AS unchanged"
