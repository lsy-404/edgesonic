...
function buildDb() {
  ...
  sqlite.exec(`
    ...
    CREATE TABLE song_instances (id TEXT PRIMARY KEY, master_id TEXT, source_id TEXT, source_type TEXT DEFAULT 'original', storage_uri TEXT, storage_object_id TEXT, suffix TEXT, content_type TEXT, size INTEGER, missing INTEGER DEFAULT 0, updated_at INTEGER DEFAULT 0);
...
async function main() {
  ...
  console.log("R2+D1 migration → copies and indexes a root-directory object:");
  ...
  const second = await app.fetch(new Request("http://test/storage/files/migrate-r2", {
    method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ cursor: firstBody.nextCursor, deleteLegacy: true }),
  }), env);
