SELECT 'albums' AS section,group_concat(id||':'||name||':'||song_count||':'||duration||':'||size,'|') AS fingerprint FROM (SELECT * FROM albums ORDER BY id);
...
SELECT 'entries' AS section,group_concat(id||':'||path||':'||display_name||':'||object_id||':'||instance_id,'|') AS fingerprint FROM (SELECT * FROM storage_entries ORDER BY id);
