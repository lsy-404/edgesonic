...
const { t } = useI18n();
const { confirmEmailChange, isLoggedIn } = useAuth();
...
onMounted(async () => {
  ...
  if (!token) {
    state.value = "error";
    error.value = t("confirmEmailChange.missingToken");
    return;
  }
  ...
  if (result.ok) {
    ...
    state.value = "error";
    error.value = result.error || t("confirmEmailChange.failed");
  }
});
</script>
    ...
    <div class="login-main">
      <div class="login-card">
        ...
        <div class="login-form">
          ...
          <p v-else-if="state === 'success'" class="login-hint">{{ t("confirmEmailChange.successHint") }}</p>
