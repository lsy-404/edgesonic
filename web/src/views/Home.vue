...
const albums = ref<Record<HomeSection, HomeAlbum[]>>({
  newest: [],
  frequent: [],
  recent: [],
});
const loading = ref<Record<HomeSection, boolean>>({
  newest: true,
  frequent: true,
  recent: true,
});
...
const sectionGeneration: Record<HomeSection, number> = {
  newest: 0,
  frequent: 0,
  recent: 0,
};
...
function msg(
  key: keyof typeof homeMessages.en,
...
function validRows(xml: string) {
  if (!isSuccessfulSubsonicResponse(xml))
    throw new Error("album list unavailable");
...
const empty = computed(
  () =>
    !anyLoading.value &&
    failed.value.size === 0 &&
    sections.every((section) => albums.value[section].length === 0),
);
const featured = computed(
  () =>
    albums.value.newest[0] ||
    albums.value.frequent[0] ||
    albums.value.recent[0],
);
...
async function loadSection(section: HomeSection, generation: number) {
  ...
  try {
    ...
    if (
      ...
      token !== sectionGeneration[section]
    )
      return;
    albums.value = {
      ...albums.value,
      [section]: albumsFromXml(validRows(xml)),
    };
    ...
    if (generation === loadGeneration && token === sectionGeneration[section])
...
async function play(album: HomeAlbum, shuffle = false) {
  ...
  try {
    ...
    if (!isSuccessfulSubsonicResponse(xml) || !tracks.length)
      throw new Error("album unavailable");
    if (!controller.signal.aborted && token === playGeneration)
...
function hideCover(id: string) {
  brokenCovers.value = new Set([...brokenCovers.value, id]);
}
function open(album: HomeAlbum) {
...
<template>
  <main class="music-home" aria-labelledby="home-title">
    <header class="heading">
      <div>
        <p class="eyebrow"><Icon name="headphones" /> EdgeSonic</p>
    ...
    <section v-if="featured || currentTrack" class="listen-panel">
      <template v-if="currentTrack"
        ><p class="eyebrow">
          <Icon name="headphones" />{{ msg("nowListening") }}
        </p>
    ...
    <div v-else class="sections">
      ...
      >
        ...
        <div
          ...
          class="grid loading-grid"
          :aria-label="msg('loadingAlbums')"
        >
        ...
        <template v-else
          ...
          <div v-if="albums[section].length" class="grid">
            <article
            ...
            >
              ...
              >
                <BudgetedImage
                  v-if="album.coverArt && !brokenCovers.has(album.id)"
                  :src="coverArtUrl(album.coverArt, 320)"
                  :alt="album.name"
                  @error="hideCover(album.id)"
                /><Icon v-else name="album" size="34" />
              </button>
              <div class="card-body">
                <button class="album-name" type="button" @click="open(album)">
                  {{ album.name }}
                </button>
                <p>{{ album.artist || "—" }}</p>
                <small>{{
                ...
                <div class="actions">
                  <button
                    type="button"
                    :disabled="playingAlbum === album.id"
                    :aria-label="msg('playAlbum', { name: album.name })"
                    @click="play(album)"
                  >
                  ...
                  ><button
                    type="button"
                    :disabled="playingAlbum === album.id"
                    :aria-label="msg('shuffleAlbum', { name: album.name })"
...
.section-error {
  ...
  border-radius: 4px;
  background: color-mix(in srgb, #d83b01 10%, var(--color-bg-secondary));
}
.play-error {
  margin: 0 0 0.8rem;
}
