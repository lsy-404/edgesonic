        ...
        <div v-for="level in [2, 1, 0]" :key="level" class="card perm-card">
          ...
          <div class="perm-list">
            <div v-for="key in permKeys" :key="key" class="perm-row" :class="{ 'perm-row-locked': isLocked(level, key) }">
              ...
              <FluentSwitch :aria-label="permLabel(key)" :model-value="permissions.find(p => p.level === level && p.name === key)?.enabled" :disabled="permissions.find(p => p.level === level && p.name === key)?.locked" @update:model-value="toggle(level, key, $event)" />
            </div>
          </div>
