...
WHERE (SELECT count(*) FROM valid_source)=11 AND NOT EXISTS (SELECT 1 FROM albums WHERE id='al-summer-vinyl-20260926')
  ...
  AND EXISTS (SELECT 1 FROM storage_objects WHERE id='obj_8e193f74e0f392d5' AND physical_key='objects/obj_8e193f74e0f392d5.jpg')
  AND (SELECT song_count FROM albums WHERE id='pending-uploads')=652;
...
WHERE NOT (
  ...
  AND (SELECT song_count FROM albums WHERE id='pending-uploads')=641
);
