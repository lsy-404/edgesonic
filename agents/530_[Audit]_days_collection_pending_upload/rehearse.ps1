...
$config = Join-Path $root 'test\summer_days_wav\wrangler.toml'
$persist = Join-Path $root 'test\summer_days_wav\.wrangler\state'
$fixture = Join-Path $root 'test\summer_days_wav\fixture.sql'
...
$mp3Rows = @(((Get-Content -Raw -LiteralPath (Join-Path $PSScriptRoot 'production_inventory.json') | ConvertFrom-Json).results | Where-Object { $_.path -like 'Days幻梦年华乐团合集/2-Summer Days/MP3(有损)/*' }))
$sql = @"
...
INSERT INTO albums(id,name,sort_name,year,cover_r2_key,song_count,duration,size) VALUES('pending-uploads','Pending Uploads','pending uploads',2012,NULL,641,141395,24700222725),('al-2b21b237dc','Summer Days','summer days',2012,'covers/al-2b21b237dc',7,0,81470371);
"@
foreach ($row in $rows) {
...
foreach ($row in $mp3Rows) {
  $master = "'$($row.master_id.Replace("'", "''"))'"; $instance = "'$($row.instance_id.Replace("'", "''"))'"; $object = "'$($row.object_id.Replace("'", "''"))'"
  $title = "'$($row.title.Replace("'", "''"))'"
  $sql += "INSERT INTO song_masters(id,album_id,artist_id,title,track,disc,duration) VALUES($master,'al-2b21b237dc','artist',$title,$($row.track),1,$($row.duration));`n"
  $sql += "INSERT INTO song_instances(id,master_id,source_id,source_type,suffix,missing,tag_scanned,storage_object_id,size) VALUES($instance,$master,'r2-local','original','mp3',0,1,$object,$($row.size));`n"
}
