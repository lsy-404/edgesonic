...
$flacScope=@(Get-Content -Raw -LiteralPath (Join-Path $base 'existing_flac_scope.json')|ConvertFrom-Json)
$flacMasterDuration=($flacScope|Measure-Object -Property master_duration -Sum).Sum
$flacInstanceDuration=($flacScope|Measure-Object -Property instance_duration -Sum).Sum
$flacSize=($flacScope|Measure-Object -Property instance_size -Sum).Sum
if($flacScope.Count -ne 14 -or $flacMasterDuration -ne 3173 -or $flacInstanceDuration -ne 3173 -or $flacSize -ne 411082170){throw 'Find-Zero FLAC aggregate snapshot changed'}
$flacGuard=@"
(SELECT COUNT(*) FROM albums WHERE id='$existing' AND name='Find-Zero' AND song_count=14 AND duration=0 AND size=411082170)=1 AND
(SELECT COUNT(*) FROM song_masters WHERE album_id='$existing')=14 AND
(SELECT COALESCE(SUM(duration),0) FROM song_masters WHERE album_id='$existing')=3173 AND
(SELECT COUNT(*) FROM song_instances si JOIN song_masters sm ON sm.id=si.master_id WHERE sm.album_id='$existing')=14 AND
(SELECT COALESCE(SUM(si.duration),0) FROM song_instances si JOIN song_masters sm ON sm.id=si.master_id WHERE sm.album_id='$existing')=3173 AND
(SELECT COALESCE(SUM(si.size),0) FROM song_instances si JOIN song_masters sm ON sm.id=si.master_id WHERE sm.album_id='$existing')=411082170
"@
$flacAfterGuard=@"
(SELECT COUNT(*) FROM albums WHERE id='$existing' AND name='Find-Zero' AND song_count=14 AND duration=3173 AND size=411082170)=1 AND
(SELECT COUNT(*) FROM song_masters WHERE album_id='$existing')=14 AND
(SELECT COALESCE(SUM(duration),0) FROM song_masters WHERE album_id='$existing')=3173 AND
(SELECT COUNT(*) FROM song_instances si JOIN song_masters sm ON sm.id=si.master_id WHERE sm.album_id='$existing')=14 AND
(SELECT COALESCE(SUM(si.duration),0) FROM song_instances si JOIN song_masters sm ON sm.id=si.master_id WHERE sm.album_id='$existing')=3173 AND
...
INSERT INTO work_queue(id,task_type,payload,status,created_at)
SELECT 'guard-find-zero-wav','metadata','{}','guard_failed',unixepoch()
WHERE (SELECT COUNT(*) FROM song_masters WHERE album_id='$target')!=25 OR (SELECT COUNT(*) FROM song_masters WHERE album_id='$target' AND disc=1)!=14 OR (SELECT COUNT(*) FROM song_masters WHERE album_id='$target' AND disc=2)!=11 OR NOT ($flacAfterGuard);
"@
$rollback=@"
WITH c($columns) AS (VALUES
$values
...
(SELECT COUNT(*) FROM albums WHERE id='$existing' AND name='Find-Zero' AND song_count=14 AND duration=0 AND size=411082170) AS flac_expected_initial_cache,
(SELECT song_count FROM albums WHERE id='$existing') AS flac_cached_song_count,
(SELECT duration FROM albums WHERE id='$existing') AS flac_cached_duration,
(SELECT size FROM albums WHERE id='$existing') AS flac_cached_size,
(SELECT COUNT(*) FROM song_masters WHERE album_id='$existing') AS flac_master_count,
(SELECT COALESCE(SUM(duration),0) FROM song_masters WHERE album_id='$existing') AS flac_master_duration,
(SELECT COUNT(*) FROM song_instances si JOIN song_masters sm ON sm.id=si.master_id WHERE sm.album_id='$existing') AS flac_instance_count,
(SELECT COALESCE(SUM(si.duration),0) FROM song_instances si JOIN song_masters sm ON sm.id=si.master_id WHERE sm.album_id='$existing') AS flac_instance_duration,
(SELECT COALESCE(SUM(si.size),0) FROM song_instances si JOIN song_masters sm ON sm.id=si.master_id WHERE sm.album_id='$existing') AS flac_instance_size,
