...
INSERT INTO assertion SELECT CASE WHEN
 ...
 NOT EXISTS(SELECT 1 FROM albums WHERE id='al-find-zero-wav') AND
 EXISTS(SELECT 1 FROM albums WHERE id='al-0494f8ac9c' AND name='Find-Zero' AND song_count=14) AND
 NOT EXISTS(SELECT 1 FROM album_display_group_members WHERE album_id IN ('al-0494f8ac9c','al-find-zero-wav')) AND
